...
func (s *session) pickCompaction() *compaction {
	...
	if v.cScore >= 1 {
		...
		// find the position of first table whose imax > cptr
		p := func() int {
		...
		if sourceLevel > 0 {
			// take key affinity into account, only for non level-0 compaction.
			n := len(tables)
		OUTER:
			for i := 0; i < n; i++ {
				t := tables[(i+p)%n]
				...
				if s.o.GetKeyAffinity(umax) >= 0 {
					t0 = append(t0, t)
					break
				}

				// negative affinity means the table might be frequently modified.
				// see if we can skip it and let it stay in source level.
				...
				// to check if it overlaps with hight levels.
